...
void LibcameraApp::ConfigureVideo(unsigned int flags)
{
	...
	cfg.bufferCount = 4; // 6 buffers is better than 4
...
bool LibcameraApp::GetVideoFrame(cv::Mat &frame)
{
	LOG(2, "Size of queue for frames to recognize before creating Mat: " << recognitionQueue.size());
	Buffer buffer = recognitionQueue.pop();

	while (recognitionQueue.size() > 0) {
		BufferPool::Shared.returnBuffer(buffer);
		buffer = recognitionQueue.pop();
	}

	const StreamInfo info = GetStreamInfo(VideoStream());
	const uint w = info.width;
	const uint h = info.height;

	frame.create(h,w,CV_8UC1);

	uint8_t *ptr = buffer.data();
	for (uint i = 0; i < h; i++, ptr += w)
	{
		memcpy(frame.ptr(i),ptr,w);
	}
