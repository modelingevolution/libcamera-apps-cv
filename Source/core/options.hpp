...
struct Options
{
	Options() : set_default_lens_position(false), af_on_capture(false), options_("Valid options are", 120, 80)
	{
		...
		options_.add_options()
			...
			("frame-counter", value<uint>(&frame_counter)->default_value(10),	
				"Every which frame to be sent to recognition.")
			("save-to-file", value<std::string>(&saveToFile)->default_value(""),
			"Save frames to binary file in given location. Name of file will be $TIMESTAMP_frames.bin.")
			;
		// clang-format on
	}
